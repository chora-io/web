        ...
        .then(res => {
          if (res.code) {
            ...
            for (let i = 0; i < proposal["proposers"].length; i++) {
              ...
              const found = res["members"].find((member: any) => member["member"]["address"] === proposer)
              if (found) {
  ...
  return (
    <div className={styles.box}>
      ...
      {proposal && !proposers && (
        <div className={styles.boxText}>
          ...
          {proposal["proposers"].map((proposer: any) => (
            <p key={proposer}>
              <Link href={`/members/?address=${proposer}`}>
                {proposer}
              </Link>
            </p>
          ))}
      ...
      <div className={styles.boxText}>
        ...
        {proposal && !policy && (
          <p>
            <Link href={`/policies/?address=${proposal["group_policy_address"]}`}>
              {proposal["group_policy_address"]}
            </Link>
          </p>
        )}
